...
export function generateProjectContext(rootDir = process.cwd(), options = {}) {
...
export function writeProjectContext(rootDir = process.cwd(), options = {}) {
...
function isNotableFile(filePath) {
    const base = path.basename(filePath).toLowerCase();
    return base.startsWith('readme') || base === 'license' || base === 'license.md' || base.endsWith('.md');
}
function walkDirectory(root, maxDepth, maxEntries) {
    let fileCount = 0;
    let dirCount = 0;
    let totalEntries = 0;
    ...
    function walk(current, depth) {
        ...
        const node = { name, path: current, isDir, children: [] };
        totalEntries++;
        if (isDir) {
            dirCount++;
            let entries = [];
...
function buildMarkdown(ctx) {
    ...
    const langLines = ctx.summary.languages.slice(0, 12).map(l => l.extension === '(none)'
        ? `- ${l.extension}: ${l.files}`
        : `- .${l.extension}: ${l.files}`);
